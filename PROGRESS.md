...
- **采购落地（2026-09-07）**：飞特 HL-1910-C001 ×15（¥1500，HLS 恒力 TTL 总线）+ Zero 3W（¥299）
  + 外壳 41 件（¥99），已花 ¥1898，整机预计 ≈¥2460 低于官方售价。
  **换舵机路线正式激活**：官方 ONNX 作废须重训、`bus.rs` 适配层、`imu_to_dxl` 走飞特固件分支、
  打印件舵机仓适配 = 头号验证项——四层影响与到货检查清单见 `docs/物料清单-进度快照.md`"一⁺"节
